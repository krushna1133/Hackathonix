...
const MAX_RETRIES = 3;
const MIN_REQUEST_GAP_MS = 2000; // min 2s between any Gemini requests
...
/**
 * Transcribe an audio chunk via Gemini with rate-limit handling.
 * Retries on 429 with exponential backoff.
 */
async function transcribeWithGemini(base64Wav: string): Promise<string> {
  // Global cooldown between all Gemini requests
  const now = Date.now();
  const gap = now - lastGeminiRequestTime;
  ...
  for (let attempt = 0; attempt < MAX_RETRIES; attempt++) {
    ...
    try {
          ...
          contents: [
            {
              parts: [
                ...
                {
                  text: "Transcribe this audio exactly as spoken. Return ONLY the spoken words — no labels, no timestamps, no commentary. If there is silence or no clear speech, return an empty string.",
                },
              ],
            },
          ],
      ...
      if (response.status === 429) {
        ...
        console.warn(
          `[Gemini] 429 rate limited, retry ${attempt + 1}/${MAX_RETRIES} in ${Math.round(backoff)}ms`,
        );
...
export class AudioRecorder {
  ...
  private lastSystemTranscript = "";
  private lastSystemTranscriptTime = 0;
  ...
  /**
   * Start polling Tauri for WASAPI-captured system audio chunks.
   * Uses a 10s poll interval + silence deduplication to avoid API flooding.
   */
  private async startSystemAudio(): Promise<void> {
    try {
      ...
      // Poll every 10 seconds (was 5s — too fast, caused rate limiting)
      this.systemAudioPollInterval = setInterval(async () => {
        ...
        try {
          ...
          // Deduplicate: skip if identical to last transcription (silence / re-capture)
          const now = Date.now();
          if (
            text === this.lastSystemTranscript &&
            now - this.lastSystemTranscriptTime < 15000
          ) {
            return;
          }

          // Also skip near-empty or very short results
          if (!text || text.replace(/\s+/g, "").length < 4) return;

          this.lastSystemTranscript = text;
          this.lastSystemTranscriptTime = now;
          ...
          console.warn("[SystemAudio] chunk error:", e);
        }
      }, 10000); // 10 second interval
    } catch (e) {
      console.warn("[SystemAudio] not available:", e);
  ...
  async start(): Promise<void> {
    ...
    try {
      ...
      this.onError?.("Microphone access denied. Please allow mic permissions.");
      this.onStatusChange?.("error");
      return;
    }

    // Start system audio capture (non-blocking, fails gracefully)
    ...
    this.recognition.onend = () => {
      // Auto-restart if still running (SpeechRecognition stops after silence)
  ...
  get hasSystemAudio() {
    return this.systemAudioAvailable;
  }
}
